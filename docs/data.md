...
It also includes a small deterministic sample dataset under `data/sample/` for reproducibility checks and examples.
...
The committed sample dataset is:

```text
data/sample/sample_instance_seed_7.json
data/sample/sample_prices_seed_7.csv
```

It is generated by:

```bash
.venv/bin/python src/data/sample_scenario.py --seed 7 --customers 12 --chargers 4 --date 2025-07-15 --out-dir data/sample
```
...
The sample scenario is intentionally small and deterministic. It contains depot settings, two vehicles, generated customers, generated chargers, and 24 hourly synthetic prices. It is not a replacement for the full processed Manhattan graph.
